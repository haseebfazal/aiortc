...
# SHM_NAME = "rtt_shm"
# RTT_SIZE = 4  # Size of an integer (4 bytes)
#
# # Initialize the shared memory globally
# try:
#     shm_rtt = shared_memory.SharedMemory(name=SHM_NAME, create=True, size=RTT_SIZE)
# except FileExistsError:
#     shm_rtt = shared_memory.SharedMemory(name=SHM_NAME)
...
class RTCRtpSender:
    ...
    async def _handle_rtcp_packet(self, packet):
        if isinstance(packet, (RtcpRrPacket, RtcpSrPacket)):
            for report in filter(lambda x: x.ssrc == self._ssrc, packet.reports):
                ...
                #     print(type(self.__rtt))
                #     struct.pack_into('f', shm_rtt.buf, 0, self.__rtt)
    ...
    async def _next_encoded_frame(self, codec: RTCRtpCodecParameters):
        ...
        if isinstance(data, Frame):
            ...
            # print(f"Frame type: {'I-frame' if force_keyframe else 'P-frame'}, Timestamp: {data.pts}")
